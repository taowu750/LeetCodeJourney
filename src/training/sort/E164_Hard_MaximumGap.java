...
public class E164_Hard_MaximumGap {
     ...
     *
     * 为了说明这一点，我们使用反证法：假设相邻数字的间距都小于 ⌈(max−min)/(N−1)⌉，并记数组排序后从小到大的数字依次为 A_1, A_2, ..., A_N，则有:
     ...
     *
     * 因此，我们可以选取整数 d=⌊(max−min)/(N−1)⌋<⌈(max−min)/(N−1)⌉。随后，我们将整个区间划分为若干个大小为 d 的桶，
     * 并找出每个整数所在的桶。根据前面的结论，能够知道，元素之间的最大间距一定不会出现在某个桶的内部，而一定会出现在不同桶当中。
     *
