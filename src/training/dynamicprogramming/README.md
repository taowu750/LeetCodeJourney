    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) {
            if (word1.charAt(i - 1) == word2.charAt(j - 1))
                ...
                // 因为这是自底向上，顺序和前面的代码是反的，所以 dp[i - 1][j] 是插入，dp[i][j - 1] 是删除 
...
这个方法对 `dp` 数组的定义是：在子串 `s[i..j]` 中，最长回文子序列的长度为 `dp[i][j]`。**这是一个范围子序列问题**。
...
总结一下，有三个细节需要注意：
1. 画出状态转移图：思考每种「状态」有哪些「选择」，这些选择是和「持有状态」相关的，所以只看「持有状态」。
其他的天数、最大交易次数状态都是随循环递进的，不用管。
2. `dp[i][k]` 是第 `i` 天、最大交易次数为 `k` 时，在第 `i` 天买入还是卖出的问题，所有只需要考虑 `prices[i]`。
3. 当买入或者卖出股票时，交易次数减一。我们用 `k-1 -> k` 的状态转移表达，它表示在原来 `k-1` 次交易的基础上又进行了一次交易，
因为买入或卖出是先后操作，所以 `dp[i][j][0]` 和 `dp[i][j][1]` 不能两个都减。
4. 因为持有会减去 `prices[i]`，所以持有的 base case 需要是 `Integer.MIN_VALUE`。
